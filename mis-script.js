...
function display(event)
	{
		...
		flag=1
		document.getElementById("prediction_text").innerHTML = "Imagen seleccionada"	
	}
...
async function predict_animal()
{
	...
	if (flag==0){
		alert("¡No se ha cargado ninguna imagen! Cargue una imagen para comenzar.")
	}
	if(flag==1){
		...
		//console.log("")
		document.getElementById("prediction_text").innerHTML = "Finalización del preprocesamiento de la imagen"

		const model = await tf.loadLayersModel('tensorflowjs-model/model.json');
		pred = model.predict(imageproc)
		pred.print()
		//console.log("")
		document.getElementById("prediction_text").innerHTML = "<p>Finalización de predicción</p>"
		new Promise(resolve => setTimeout(resolve, 1000))

		//Declaración del arreglo con las clases de nuestro modelo
		animals = ["Ajolote", "Águila Calva", "Luciérnaga", "Mariposa Monarca", "Oso Polar","Zorro","Águila Real","León Marino","Vaquita Marina","Zebra"]
		...
		pred.data()
			.then((data) => {console.log(data)
				...
				document.getElementById("prediction_text").innerHTML = "Reconocimiento completado"					
